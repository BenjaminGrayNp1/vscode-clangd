...
class MemoryUsageFeature implements vscodelc.StaticFeature {
  constructor(private context: ClangdContext) {
    ...
    adapter.onDidChangeTreeData((e) => vscode.commands.executeCommand(
                                    'setContext', 'clangd.asm.memoryUsage.hasData',
                                    adapter.root !== undefined));
    this.context.subscriptions.push(
        vscode.window.registerTreeDataProvider('clangd.asm.memoryUsage', adapter));
    this.context.subscriptions.push(
        vscode.commands.registerCommand('clangd.asm.memoryUsage', async () => {
          const usage =
              await this.context.client.sendRequest(MemoryUsageRequest, {});
          adapter.root = convert(usage, '<root>');
        }));
    this.context.subscriptions.push(vscode.commands.registerCommand(
        'clangd.asm.memoryUsage.close', () => adapter.root = undefined));
  }
  ...
  initialize(capabilities: vscodelc.ServerCapabilities,
    ...
    vscode.commands.executeCommand('setContext', 'clangd.asm.memoryUsage.supported',

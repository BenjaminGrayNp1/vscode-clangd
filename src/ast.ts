...
class ASTFeature implements vscodelc.StaticFeature {
  constructor(private context: ClangdContext) {
    ...
    const tree =
        vscode.window.createTreeView('clangd.asm.ast', {treeDataProvider: adapter});
    context.subscriptions.push(
        ...
        adapter.onDidChangeTreeData((_) => {
          vscode.commands.executeCommand('setContext', 'clangd.asm.ast.hasData',
                                         adapter.hasRoot());
        ...
        vscode.commands.registerCommand(
            'clangd.asm.ast.close', () => adapter.setRoot(undefined, undefined)));
  }
  ...
  initialize(capabilities: vscodelc.ServerCapabilities,
    ...
    vscode.commands.executeCommand('setContext', 'clangd.asm.ast.supported',
